...
const TextEditor = ({ onChange, inputKey, value, hasUploaded }: Props) => {
  ...
  const editor = useEditor({
    ...
    content: value, // Initially setting the content from state
    onUpdate({ editor }) {
  ...
  const buttonClass = (isActive: boolean) =>
    `text-[14px] px-2 py-1 rounded-[10px] ${
      isActive ? "bg-blue text-white" : "bg-[#eee] text-black"
    }`;
  ...
  return (
    <div className="">
      <div className="flex flex-wrap border border-blue rounded-tl-[10px] rounded-tr-[10px] p-2 gap-1 bg-[#eee]">
        <button
          type="button"
