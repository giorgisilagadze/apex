...
export default function EmployeeCard({ item, forRender, setForRender }: Props) {
  ...
  const handleDelete = async () => {
    if (!isLoading) {
      setIsLoading(true);
      try {
        const response = await axiosAdmin.delete(`/employ/${item.id}`);
      ...
      <div className=" w-full flex flex-col border-x border-b border border-[#eee] mt-4 rounded-[5px]">
        <div className="w-full p-6 grid grid-cols-4 gap-5 items-center border-b border-[#eee]">
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/${locale}/admin/adminPanel/employees/${item.id}`}
